...
import './rolecas.css';
import OrganizationMemberRoleStore from '../../stores/organization/memberRole/MemberRoleStore';

let enterItem;

@inject('AppState')
@observer
class ReRoleCas extends Component {
  constructor(props) {
    super(props);
    this.state = {
      // 弹出层显示与否
  ...
  mouseEnterCanChose = (index, name) => {
    // 当前组织可分配的权限
    // enterItem = Observable.fromPromise(new Promise((resolve, reject) => {
    //   this.setState({
    //     mouseLeave: false,
    //   });
    //   setTimeout(() => {
    //     const canChoseList = OrganizationMemberRoleStore.getRoleData;
    //     window.console.log(index);
    //     const secondeLi = canChoseList.filter(item => item.serviceName === name)[0].roles;
    //     OrganizationMemberRoleStore.setSecondLi(secondeLi);
    //     this.setState({
    //       secondTop: index * 30,
   ...
   * @memberof ReRoleCas
   */
  mouseLeaveCanChose = (index, name) => {
    // enterItem.unsubscribe();
   ...
   */
  showSecondCanChose = () => {
    const secondList = OrganizationMemberRoleStore.getSecondLi
   ...
   */
  showCanChose = () => {
    const storeFirstLi = OrganizationMemberRoleStore.getRoleData;
  ...
  handleSave = () => {
    if (!this.props.add) {
      ...
      OrganizationMemberRoleStore.handleRoleSave(currentMenuType.id, role)
        .then(() => {
      ...
      OrganizationMemberRoleStore.setAddChosenRoles(
  ...
  showDropDown = () => {
    ...
    const RoleCas = (
      ...
      >
        ...
        {/* 底部的保存和取消按钮 */}
        <Permission service={'hap-user-service.member-role-organization.update'} type={type} organizationId={organizationId}>
          <div
            style={{
              display: 'flex',
   ...
   */
  handleCancel = () => {
    const data = this.props.add ? {} : OrganizationMemberRoleStore.getOriginMemberRole
