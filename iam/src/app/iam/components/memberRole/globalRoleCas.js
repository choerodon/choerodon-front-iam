...
import './rolecas.css';
import GlobalMemberRoleStore from '../../stores/globalStores/globalMemberRole';
...
class ReRoleCas extends Component {
   ...
   */
  showSecondCanChose = () => {
    const secondList = GlobalMemberRoleStore.getSecondLi
   ...
   */
  showCanChose = () => {
    const storeFirstLi = GlobalMemberRoleStore.getRoleData;
  ...
  handleSave = () => {
    if (!this.props.add) {
      ...
      GlobalMemberRoleStore.handleRoleSave(currentMenuType.id, role)
        .then(() => {
      ...
      GlobalMemberRoleStore.setAddChosenRoles(
  ...
  showDropDown = () => {
    ...
    const RoleCas = (
      ...
      >
        ...
        {/* 底部的保存和取消按钮 */}
        <Permission service={'hap-user-service.member-role-global.update'} type={type} organizationId={organizationId}>
          <div
            style={{
              display: 'flex',
   ...
   */
  handleCancel = () => {
    const data = this.props.add ? {} : GlobalMemberRoleStore.getOriginMemberRole
