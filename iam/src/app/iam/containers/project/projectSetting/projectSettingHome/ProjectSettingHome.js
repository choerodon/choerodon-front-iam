...
class ProjectSettingHome extends Component {
  ...
  render() {
    ...
    return (
      <Page>
        <Permission service={['iam-service.project.query']} organizationId={AppState.currentMenuType.organizationId}
          type={AppState.currentMenuType.type} projectId={AppState.currentMenuType.id} >
          <Header title="项目信息">
            {projectInfo.enabled ? (
              <Permission service={['iam-service.project.disableProject']} organizationId={orgId} type={type} projectId={proId}>
                <Button
                  icon="remove_circle_outline"
                  onClick={this.handleAble.bind(this, projectInfo)}
                >
                  停用
                </Button>
              </Permission>
        ...
        <Content
          ...
          link="http://v0-5.choerodon.io/zh/docs/user-guide/system-configuration/project/pro_info/"
        >
              ...
              {projectInfo.enabled ? (
                <Permission service={['iam-service.project.update']} type={type} organizationId={orgId} projectId={proId}>
                  <div className="btnGroup">
                    ...
                    <Button funcType="raised" className={'cancel'}
                      onClick={this.cancelValue}>{Choerodon.languageChange('cancel')}</Button>
                  </div>
                </Permission>
              ) : (
                  <Permission service={['iam-service.project.update']} type={type} organizationId={orgId} projectId={proId}>
