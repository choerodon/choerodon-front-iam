...
@store('GlobalMemberRoleStore')
class GlobalMemberRoleStore2 {
  @observable roleData = [];
  ...
  axiosGetRoleSearchTable(orgId, id, page) {
    this.setSpinRole(true);
    axios.get(`/uaa/v1/memberRoles/global?roleId=${id}&page=${page-1}&size=10`).then((data) => {
  ...
  loadRoles(id) {
    axios.get('/uaa/v1/roles/selectGlobalRole/service')
      .then((data) => {
  ...
  loadRolesByRoles(id, page, page2) {
    axios.get(`/uaa/v1/roles/selectGlobalRole`)
      .then((data) => {
  ...
  loadMemberRoles(organizationId, page, state) {
    ...
    let url = '';
    if (!state) {
      url = `/uaa/v1/memberRoles/global?page=${page-1}&size=10`;
    } else if (state.code === '') {
      url = `/uaa/v1/memberRoles/global?page=${page-1}&size=10&param=${state.input}`;
    } else {
      url = `/uaa/v1/memberRoles/global?page=${page-1}&size=10&${state.code}=${state.input}`;
    }
  ...
  reHandleDelete(id, userId) {
    return Promise.resolve(axios.delete(`/uaa/v1/memberRoles/global/users/${userId}`));
  }

  handleBatchDelete(id, success, total, fn) {
    axios.delete(`/uaa/v1/memberRoles/global/users/${id}`)
      .then(() => {
        success += 1;
        if (success === total) {
          fn;
  ...
  handleRoleSave(id, role) {
    return axios.put(`/uaa/v1/memberRoles/global`, JSON.stringify(role));
  }
  // handleRoleSave(role) {
  ...
  handleAddOk(id, data, success, total, fn, fnTwo) {
    return axios.post('/uaa/v1/memberRoles/global', JSON.stringify(data));
  }
  handleSearch(id, value) {
    const that = this;
    axios.get('/uaa/v1/memberRoles/global')
      .then(data => {
        if (data) {
...
const GlobalMemberRoleStore = new GlobalMemberRoleStore2();
